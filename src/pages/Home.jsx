...
import { TRACKS, COURSES, coursesInTrack } from '../data/curriculum.js'
import CourseCard from '../components/CourseCard.jsx'
import { useStore, rankFor, nextRank } from '../store/useStore.js'
  ...
  return (
    <div>
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-cblue-800 via-cblue to-violet-700 text-white">
        ...
        <div className="container-x relative grid items-center gap-8 py-16 lg:grid-cols-2">
          <div>
            ...
            >
              🚀 Zero → Chief Risk Officer
            </motion.span>
            <motion.h1
              initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}
              className="mt-4 text-4xl font-extrabold leading-tight sm:text-5xl"
            >
              Master Governance, Risk & Compliance — the hands-on way.
            </motion.h1>
            <motion.p
              initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
              className="mt-4 max-w-xl text-lg text-white/85"
            >
              A guided path through {COURSES.length} courses and {totalLessons}+ lessons, with
              real-world scenario labs, XP, and ranks. Learn ISO 27001, NIST, SOC 2, GDPR,
              AI governance and enterprise GRC — by <em>doing</em>, not just reading.
            </motion.p>
            <motion.div
              initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}
              className="mt-7 flex flex-wrap gap-3"
            >
              <Link to="/roadmap" className="btn bg-white text-cblue-700 hover:bg-white/90">
          ...
          >
            ...
            <div className="mt-5 text-sm text-white/80">{xp} XP {next && `· ${next.xp - xp} to ${next.name}`}</div>
            <div className="mt-2 h-2 overflow-hidden rounded-full bg-white/20">
              <div className="h-full rounded-full bg-gradient-to-r from-neon-green to-neon-cyan transition-all"
                style={{ width: `${next ? Math.min(100, (xp / next.xp) * 100) : 100}%` }} />
            </div>
            <div className="mt-6 grid grid-cols-3 gap-2 text-center text-xs">
              {[['🧭', 'Learn'], ['🎮', 'Play labs'], ['🏆', 'Rank up']].map(([e, t]) => (
                <div key={t} className="rounded-lg bg-white/10 py-3">
                  <div className="text-xl">{e}</div><div className="mt-1 text-white/70">{t}</div>
                </div>
              ))}
            </div>
          </motion.div>
      ...
      <section className="container-x grid gap-4 py-12 sm:grid-cols-3">
        {[
          { i: <RouteIcon className="text-cblue" />, t: 'A real roadmap', d: 'Seven levels from GRC Foundations to CRO. No guessing what to learn next.' },
          { i: <Gamepad2 className="text-violet-600" />, t: 'Play the scenarios', d: 'Beat an incident timer, hunt cloud flaws, score a risk register — learn by doing.' },
          { i: <ShieldCheck className="text-emerald-600" />, t: 'Every major framework', d: 'ISO 27001 & 42001, NIST CSF & AI RMF, SOC 2, PCI, GDPR, SOX, TPRM and more.' },
        ].map((f) => (
      ...
      <section className="container-x pb-8">
        <div className="mb-4 flex items-end justify-between">
          <div>
            <h2 className="text-2xl font-extrabold">Explore the path</h2>
            <p className="text-cslate">Courses grouped by level — start anywhere, but the roadmap is the fastest route.</p>
          </div>
          <Link to="/roadmap" className="hidden text-sm font-semibold text-cblue hover:underline sm:block">See full roadmap →</Link>
        </div>
          ...
          return (
            <div key={track.id} className="mb-8">
              <div className="mb-3 flex items-center gap-2">
                <span className={`rounded-md bg-gradient-to-r ${track.color} px-2 py-0.5 text-xs font-bold text-white`}>{track.level}</span>
        ...
        <div className="text-center">
          <Link to="/catalog" className="btn-ghost">See all {COURSES.length} courses</Link>
        </div>
      </section>
    </div>
  )
