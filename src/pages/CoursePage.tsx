...
import { COURSE_BY_ID, CERT_BY_ID, courseLessonCount, allLessons } from '../data/curriculum'
...
const typeIcon = { reading: FileText, quiz: HelpCircle, lab: Gamepad2, video: PlayCircle } as const
...
function LessonRow({ lesson }: { lesson: Lesson }) {
  ...
  const done = lesson.type === 'lab' ? (labsDone[lesson.labId as string] || 0) > 0 : Boolean(lessonsDone[lesson.id])
  return (
    <button
...
export default function CoursePage() {
  ...
  const course = courseId ? COURSE_BY_ID[courseId] : undefined
  const [open, setOpen] = useState(0)
  const nav = useNavigate()
  const enrolled = useStore((s) => Boolean(courseId && s.enrolled[courseId]))
  ...
  const start = () => {
    enroll(course.id)
    if (firstLesson) nav(`/learn/${firstLesson.id}`)
  }
        ...
        <div className="container-x py-10">
          <div className="mb-3 text-sm text-white/70">
            ...
            {cert && (
              <>
                {' / '}
                <Link to={`/cert/${cert.id}`} className="hover:underline">
                  {cert.code}
                </Link>
              </>
            )}
          ...
          <div className="grid gap-6 lg:grid-cols-3">
            <div className="lg:col-span-2">
              ...
              <div className="mt-5 flex flex-wrap items-center gap-5 text-sm">
                ...
                <span className="flex items-center gap-1">
                  <BookOpen size={15} /> {courseLessonCount(course)} lessons
                </span>
              </div>
            </div>
            ...
            <div className="rounded-2xl bg-white p-5 text-cink shadow-cardhover">
              {enrolled ? (
                ...
                <>
                  ...
                  <p className="mt-3 text-xs text-cslate">
                    Includes {courseLessonCount(course)} lessons, hands-on labs, and XP toward your rank.
                  </p>
                </>
              )}
            </div>
      ...
      <div className="container-x grid gap-8 py-10 lg:grid-cols-3">
        <div className="lg:col-span-2">
          ...
          <p className="text-sm text-cslate">
            {course.modules.length} modules · {courseLessonCount(course)} lessons
          </p>
          ...
          <div className="card p-5">
            ...
            <ul className="mt-3 space-y-2 text-sm text-cslate">
              <li className="flex gap-2">
                <Gamepad2 size={16} className="text-violet-600" /> Hands-on scenario labs
              </li>
              <li className="flex gap-2">
                <HelpCircle size={16} className="text-cblue" /> Knowledge checkpoints
              </li>
              <li className="flex gap-2">
                <CheckCircle2 size={16} className="text-emerald-500" /> XP &amp; rank progression
              </li>
              <li className="flex gap-2">
                <FileText size={16} className="text-cslate" /> Original, exam-aligned notes
              </li>
            </ul>
          </div>
        </aside>
