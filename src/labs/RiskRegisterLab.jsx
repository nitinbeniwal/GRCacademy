...
const SCEN = {
  ...
  inherentL: 4, inherentI: 5,
  control: 'Enforce phishing-resistant MFA on all admin accounts',
  residualL: 2, residualI: 5,
  treatment: 'mitigate',
}

function band(s) { return s <= 4 ? 'Low' : s <= 9 ? 'Medium' : s <= 15 ? 'High' : 'Critical' }
function color(s) { return s <= 4 ? '#28e07a' : s <= 9 ? '#fbbf24' : s <= 15 ? '#fb923c' : '#f43f5e' }

export default function RiskRegisterLab({ onComplete }) {
  return (
    <LabShell labId="risk-register" title="LAB: Risk Register"
...
function Slider({ label, val, set }) {
  return (
    <div>
      <div className="mb-1 flex justify-between text-xs text-slate-400"><span>{label}</span><span className="font-bold text-white">{val}</span></div>
      <input type="range" min="1" max="5" value={val} onChange={(e) => set(+e.target.value)} className="w-full accent-cyan-400" />
    </div>
  )
}

function Flow({ submit }) {
  const [stage, setStage] = useState(0)
  const [iL, setIL] = useState(3); const [iI, setII] = useState(3)
  const [rL, setRL] = useState(3); const [rI, setRI] = useState(3)
    ...
    <div>
      ...
      {stage === 0 && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
          ...
          <div className="mt-3 text-sm">Score: <b style={{ color: color(iScore) }}>{iScore} · {band(iScore)}</b></div>
          <button onClick={submitInherent} className="btn-primary mt-4">Lock inherent score →</button>
        </motion.div>
      )}
      ...
      {stage === 1 && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
          ...
          <div className="mt-2 text-xs text-slate-400">Tip: MFA cuts likelihood hard, but impact of admin compromise stays high.</div>
          <div className="mt-2 text-sm">Residual: <b style={{ color: color(rScore) }}>{rScore} · {band(rScore)}</b></div>
          <button onClick={submitResidual} className="btn-primary mt-4">Lock residual score →</button>
        </motion.div>
      )}
      ...
      {stage === 2 && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
          ...
          <div className="grid grid-cols-2 gap-3">
            {[['mitigate', 'Mitigate'], ['accept', 'Accept'], ['transfer', 'Transfer'], ['avoid', 'Avoid']].map(([id, l]) => (
              <button key={id} onClick={() => chooseTreatment(id)}
          ...
          <div className="mt-3 text-xs text-slate-400">We added a control to reduce the risk — which treatment does that represent?</div>
        </motion.div>
      )}
    </div>
  )
