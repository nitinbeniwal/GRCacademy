...
export default function ShadowAiLab({ onComplete }: { onComplete?: () => void }) {
  return (
    <LabShell
      labId="shadow-ai"
      title="LAB: Shadow AI Discovery"
...
function Scan({ submit }: { submit: (score: number) => void }) {
  const [i, setI] = useState(0)
  const [correct, setCorrect] = useState(0)
  const [flash, setFlash] = useState<'ok' | 'no' | null>(null)
  const item = ITEMS[i]!

  const decide = (isAI: boolean) => {
    if (flash) return
  ...
  return (
    <div>
      <div className="mb-4 flex items-center justify-between text-sm text-slate-400">
        <span>
          Scanning… {i + 1}/{ITEMS.length}
        </span>
        <span>
          Registry entries logged: <b className="text-neon-green">{correct}</b>
        </span>
      </div>
