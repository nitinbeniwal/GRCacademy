...
export default function PolicyLab({ onComplete }: { onComplete?: () => void }) {
  return (
    <LabShell
      labId="policy"
      title="LAB: Policy Builder"
      brief="📜 Assemble an enforceable Access & Security Policy. KEEP clauses with clear, mandatory ‘shall/must’ requirements. REJECT vague filler."
...
function Build({ submit }: { submit: (score: number) => void }) {
  ...
  return (
    <div>
      <div className="mb-4 flex items-center justify-between text-sm text-slate-400">
        <span>
          Clause {i + 1}/{CLAUSES.length}
        </span>
        <span>
          Enforceable clauses kept: <b className="text-neon-green">{correct}</b>
        </span>
      </div>
      <motion.div
        key={i}
      ...
      >
        “{c.t}”
        {flash && (
          <div className={`mt-3 text-sm font-bold ${flash === 'ok' ? 'text-neon-green' : 'text-neon-red'}`}>
