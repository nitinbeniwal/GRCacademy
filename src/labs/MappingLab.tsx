...
export default function MappingLab({ onComplete }: { onComplete?: () => void }) {
  return (
    <LabShell
      labId="mapping"
      title="LAB: Control Mapping — NIST AI RMF"
      brief="🧩 Map each control to the correct NIST AI RMF function. Tap a control, then tap its function."
...
function Map({ submit }: { submit: (score: number) => void }) {
  const [sel, setSel] = useState<number | null>(null)
  const [placed, setPlaced] = useState<Record<number, string>>({})
  const [correct, setCorrect] = useState(0)

  const assign = (fid: string) => {
  ...
  return (
    <div>
      <div className="mb-3 text-sm text-slate-400">
        Placed {Object.keys(placed).length}/{CONTROLS.length}
      </div>
            ...
            return (
              <span
                ...
                className={`rounded-lg px-3 py-2 text-xs ${
                  right ? 'bg-neon-green/15 text-emerald-300 line-through' : 'bg-neon-red/15 text-rose-300'
                }`}
              >
                {c.t}
              </span>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        {FUNCS.map((f) => {
          const count = Object.entries(placed).filter(([k, v]) => v === f.id && CONTROLS[Number(k)]!.f === f.id).length
          return (
            <motion.button
              key={f.id}
              whileHover={{ scale: sel !== null ? 1.04 : 1 }}
              onClick={() => assign(f.id)}
              disabled={sel === null}
            ...
            >
              <div className="text-sm font-extrabold" style={{ color: f.c }}>
                {f.label}
              </div>
