...
// Dark "game zone" wrapper. Children get a `submit(score)` via render prop.
export default function LabShell({ labId, title, brief, children, onComplete }) {
  ...
  const replay = () => { setScore(null); setRunKey((k) => k + 1) }
        ...
        {score === null ? (
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="py-8 text-center">
            ...
            <p className="mt-2 text-slate-300">
              {score >= 85 ? 'Outstanding — you nailed it.' : score >= 60 ? 'Passed. Solid work.' : 'Not quite 60% — replay to master it.'}
            </p>
