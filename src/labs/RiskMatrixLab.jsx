...
function cellColor(l, i) { return band(l * i).c }

export default function RiskMatrixLab({ onComplete }) {
  return (
    <LabShell labId="risk-matrix" title="LAB: Risk Assessment Matrix"
...
function Game({ submit }) {
  ...
  const choose = (l, i) => {
    ...
    setTimeout(() => {
      ...
      else { setRound(round + 1); setPick(null); setLocked(false) }
    }, 1700)
  }

  return (
    <div>
      <div className="mb-4 text-sm text-slate-400">Scenario {round + 1} / {ROUNDS.length}</div>
      <div className="mb-5 rounded-xl border border-night-600 bg-night-800 px-4 py-3 text-slate-200">{r.txt}</div>

      <div className="flex gap-4">
        {/* y label */}
        <div className="flex flex-col items-center justify-center">
          <span className="rotate-180 text-xs font-bold tracking-wide text-slate-400" style={{ writingMode: 'vertical-rl' }}>IMPACT →</span>
        </div>
          ...
          <div className="grid grid-cols-5 gap-1.5">
            {[5, 4, 3, 2, 1].map((i) =>
              [1, 2, 3, 4, 5].map((l) => {
                ...
                return (
                  <motion.button key={`${l}-${i}`} whileHover={{ scale: locked ? 1 : 1.08 }}
                    onClick={() => choose(l, i)} disabled={locked}
                    className="relative aspect-square rounded-md text-[11px] font-bold text-black/70"
                    style={{ background: cellColor(l, i), opacity: locked && !isPick && !isAns ? 0.35 : 1,
      ...
      {locked && pick && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
          className="mt-4 rounded-lg bg-night-800 px-4 py-3 text-sm">
          You picked <b style={{ color: cellColor(pick.l, pick.i) }}>{band(pick.l * pick.i).t}</b> ({pick.l * pick.i}).
          Expert: <b style={{ color: cellColor(r.L, r.I) }}>{band(r.L * r.I).t}</b> ({r.L * r.I}).
        </motion.div>
      )}
    </div>
  )
