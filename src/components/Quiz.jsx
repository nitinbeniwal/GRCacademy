...
function Question({ lessonId, q, index, onResolved }) {
  const markQuiz = useStore((s) => s.markQuiz)
  const already = useStore((s) => s.quizCorrect[`${lessonId}|${index}`])
  const [picked, setPicked] = useState(null)
  const [val, setVal] = useState('')
  const [status, setStatus] = useState(already ? 'correct' : null) // correct | wrong
  const [showHint, setShowHint] = useState(false)

  const resolve = (correct) => {
    setStatus(correct ? 'correct' : 'wrong')
    if (correct) { markQuiz(lessonId, index, true); onResolved?.() }
  }

  const checkInput = () => {
    const answers = (Array.isArray(q.answer) ? q.answer : [q.answer]).map(norm)
  ...
  return (
    <div className="rounded-xl border border-cline bg-white p-4">
      <div className="mb-3 flex items-start gap-2 font-semibold">
        <span className="grid h-6 w-6 shrink-0 place-items-center rounded-full bg-cblue-50 text-xs text-cblue">{index + 1}</span>
        <span>{q.q}</span>
      </div>
      ...
      {q.type === 'mc' ? (
          ...
          {q.options.map((opt, oi) => {
            ...
            return (
              <button
                key={oi}
                disabled={done}
                onClick={() => { setPicked(oi); resolve(oi === q.answer) }}
        ...
        <div className="flex flex-wrap gap-2">
          <input
            value={val} onChange={(e) => setVal(e.target.value)} disabled={done}
          ...
          />
          {!done && <button onClick={checkInput} className="btn-primary">Check</button>}
        </div>
      )}
      ...
      {status && (
        <div className={`mt-3 flex items-start gap-2 rounded-lg px-3 py-2 text-sm
          ...
          <span><b>{done ? 'Correct!' : 'Not quite.'}</b> {q.explain}</span>
        </div>
      )}

      {!done && q.type === 'input' && (
        <button onClick={() => setShowHint(true)} className="mt-2 flex items-center gap-1 text-xs font-semibold text-amber-600">
          <Lightbulb size={13} /> Hint
        </button>
      )}
      {showHint && !done && <div className="mt-1 text-xs text-cslate">First letter: <b>{(Array.isArray(q.answer) ? q.answer[0] : q.answer)[0]}</b></div>}
    </div>
  )
}

export default function Quiz({ lessonId, questions, onAllCorrect }) {
  ...
  return (
    <div>
      <div className="mb-4 flex items-center justify-between">
        ...
        <span className="text-sm text-cslate">{solved}/{questions.length} correct</span>
      </div>
      <div className="space-y-4">
        {questions.map((q, i) => (
          <Question key={i} lessonId={lessonId} q={q} index={i} onResolved={() => {
